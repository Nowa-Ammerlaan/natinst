...
rpm_location="yum.repos.intel.com/oneapi/"
...
wget -nc "https://${rpm_location}repodata/repomd.xml"
primary_with_hash=$(cat "repomd.xml" | sed -n -e 's/^.*location href="//p' | sed -n -e 's/"\/>//p' | grep primary.xml.gz)
wget -nc "https://${rpm_location}${primary_with_hash}"
primary_filename="$(basename ${primary_with_hash//.gz})"
gunzip "${primary_filename}.gz"
list_rpms=$(cat "${primary_filename}" | sed -n -e 's/^.*location href="//p' | sed -n -e 's/"\/>//p')
